...
import { Shield, BookOpen, Database, Code, Terminal, Layers, Grid, Sliders, RefreshCw, Bot, Zap, Menu, X } from "lucide-react";
...
export default function App() {
  ...
  // Función para cerrar menú al cambiar vista en móvil
  ...
  return (
    <div className="flex h-screen bg-[#070b19] text-slate-100 font-sans overflow-hidden">
      ...
      {/* SIDEBAR - Responsive */}
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-[#0a1023] border-r border-slate-800/80 p-4 flex flex-col transform transition-transform duration-300 md:relative md:translate-x-0 ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex items-center justify-between px-2 mb-8 mt-2">
            <div className="flex items-center gap-3">
                ...
                <div>
                    ...
                    <span className="text-[10px] font-mono text-emerald-500 mt-1 block">ISP Audit Portal</span>
                </div>
            </div>
        ...
        <div className="text-[10px] font-mono text-slate-500 uppercase tracking-wider px-2 mb-3">Módulos</div>
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-xs font-medium transition-all ${
                  isActive ? "bg-red-600 text-white font-bold shadow-lg shadow-red-600/20" : "text-slate-400 hover:bg-slate-800/60"
                }`}
              >
                <Icono size={16} />
      ...
      <main className="flex-1 flex flex-col overflow-y-auto bg-[#070b19]">
        <header className="h-16 border-b border-slate-800/80 px-4 md:px-8 flex items-center justify-between shrink-0 bg-[#0a1023]/50 backdrop-blur">
          <div className="flex items-center gap-3">
          ...
          <div className="hidden md:flex items-center gap-2 px-3 py-1 bg-slate-900/80 border border-slate-800 rounded-full font-mono text-xs text-slate-300">
            <Zap size={14} className="text-red-500" />
            <span>Nivel: <strong className="text-slate-400">LOW</strong></span>
          </div>
        </header>
        ...
        <div className="p-4 md:p-8 max-w-6xl mx-auto w-full flex-1 flex flex-col">
          ...
          <footer className="mt-auto pt-12 pb-4 text-center font-mono text-xs">
            <a href="https://github.com/andresasd952-design" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-slate-950/80 border border-slate-800 text-slate-400 hover:text-cyan-400 hover:border-slate-700 transition-all">
